---
description: "llm-health command: med-review. Route to the local-first health concierge workflow with alias-only privacy, typed artifacts, diagnostic gaps, and research queues."
...
Use the health skill at `skills/health-concierge/SKILL.md`. Resolve the local `llm-health` repo/store, activate `.venv` if present, and map this slash command to the equivalent `health med-review` or natural-language intent.
